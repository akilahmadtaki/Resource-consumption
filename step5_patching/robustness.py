...
def patched_length(styled_text, plain_vec):
    def patch_hook(module, inputs, output):
        hidden = output[0] if isinstance(output, tuple) else output
        if hidden.shape[1] > 1:                 # only first pass (full prompt)
...
# build pairs: 3 easy puzzles x 2 styles (poetic, archaic)
test_pairs = []
for idx in [0, 1, 2]:                            # first 3 easy puzzles
    plain_text = puzzles.easy_puzzles[idx]
    for (pidx, style, styled_text) in styled_easy:
        if pidx == idx and style in ("poetic", "archaic"):
            test_pairs.append((idx, style, plain_text, styled_text))

print(f"{'puzzle':>7} {'style':>8} {'base':>6} {'patched':>8} {'reduction':>10}")
print("-" * 45)
reductions = []
for idx, style, plain_text, styled_text in test_pairs:
    base, _ = measure_reasoning(styled_text, max_new_tokens=4000)
    if base is None:
        print(f"easy{idx:>3} {style:>8} {'None':>6}  (styled looped - skip)")
        continue
    pvec = capture_plain(plain_text)
    patched = patched_length(styled_text, pvec)
    if patched is None:
        print(f"easy{idx:>3} {style:>8} {base:>6} {'None':>8}  (patched broke)")
        continue
    red = 100 * (base - patched) / base
    reductions.append(red)
    print(f"easy{idx:>3} {style:>8} {base:>6} {patched:>8} {red:>9.0f}%")

if reductions:
    import statistics
    print("-" * 45)
    print(f"mean reduction: {statistics.mean(reductions):.0f}%  "
          f"(over {len(reductions)} pairs)")
